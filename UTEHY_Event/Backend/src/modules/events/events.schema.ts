...
// ── Base object (có refine kiểm tra ngày tháng) ─────────────────
const eventBaseSchema = z.object({
  ...
  checkin_radius_m: z.number().int().min(50).max(1000).default(200), // Required
  start_time: z.string().datetime('Thời gian bắt đầu không hợp lệ'),
  end_time: z.string().datetime('Thời gian kết thúc không hợp lệ'),
  registration_deadline: z.string().datetime('Hạn đăng ký không hợp lệ'),
...
// ── UPDATE: optional fields + validate nếu có thay đổi time ─────
export const updateEventSchema = eventBaseSchema.merge(
  z.object({
