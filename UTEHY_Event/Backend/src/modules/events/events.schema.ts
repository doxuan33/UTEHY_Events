...
// ── Base object (không có refine) dùng cho UPDATE ────────────
const eventBaseSchema = z.object({
  category_id: z.number().int().positive().optional(),
  title: z.string().min(5, 'Tiêu đề phải có ít nhất 5 ký tự').max(255),
  description: z.string().optional(),
  banner_url: z.string().url('URL ảnh bìa không hợp lệ').optional(),
...
// ── CREATE: có refine kiểm tra ngày tháng ────────────────────
...
// ── UPDATE: partial trên base (không có refine) ───────────────
