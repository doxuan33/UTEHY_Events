...
export const StudentManagement = () => {
  ...
  const handleToggleActive = async (student: any) => {
    ...
    if (!window.confirm(`Bạn có chắc chắn muốn ${action} tài khoản của sinh viên ${student.profile?.full_name}?`)) return;
    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (let i = 0; i < data.length; i++) {
          const row = data[i];
          const studentId = (row['Mã sinh viên'] || row['MSSV'] || row['student_id'])?.toString().trim();
          const fullName = (row['Họ và tên'] || row['HoTen'] || row['Full Name'] || row['full_name'])?.toString().trim();
          const email = (row['Email'] || (studentId ? `${studentId}@student.utehy.edu.vn` : ''))?.toString().trim();
          const className = (row['Lớp'] || row['Lop'] || row['Class'])?.toString().trim();
          const faculty = (row['Khoa'] || row['Faculty'])?.toString().trim();
          const phone = (row['Số điện thoại'] || row['SoDienThoai'] || row['Phone'])?.toString().trim();
          ...
          const validationErrors = validateStudentData({ studentId, fullName, email, phone: phoneStr });
          
          if (validationErrors.length > 0) {
            failCount++;
            logs.push({
              studentId: studentId || `Dòng ${i + 2}`,
          ...
          try {
            const password = studentId!.length >= 8 ? `S${studentId}` : `Student@${studentId}`;
            ...
            // If rate limited, stop and inform user
            if (err.response?.status === 429) {
              alert('Hệ thống đang bận (Quá nhiều yêu cầu). Vui lòng thử lại sau 15 phút.');
              break; 
            }
          }
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="p-10 border-b border-gray-50 text-center flex-shrink-0">
                ...
                <p className="text-sm font-bold text-gray-400 mt-2">Tải lên file Excel (.xlsx, .xls) để tạo tài khoản hàng loạt.</p>
              </div>

              <div className="p-10 space-y-8 overflow-y-auto flex-1">
                {!isImporting && importLogs.length === 0 ? (
                  <div className="space-y-6">
                    <div className="p-6 border-2 border-dashed border-gray-200 rounded-[32px] text-center hover:border-blue-500 hover:bg-blue-50 transition-all group relative cursor-pointer">
                      <input
                        type="file"
                        accept=".xlsx, .xls"
                      ...
                      />
                      <Upload className="h-8 w-8 text-gray-300 mx-auto mb-3 group-hover:text-blue-500 transition-colors" />
                      <p className="text-sm font-black text-gray-900">Chọn file từ máy tính</p>
                      <p className="text-xs text-gray-400 mt-1">Kéo thả hoặc click để chọn file</p>
                    </div>
                    
                    <div className="bg-blue-50 p-6 rounded-[24px] space-y-3">
                      <p className="text-xs font-black text-blue-600 uppercase tracking-widest">Yêu cầu định dạng file:</p>
                      <ul className="text-xs text-blue-700 font-medium space-y-1.5">
                        <li className="flex items-center"><div className="h-1 w-1 bg-blue-400 rounded-full mr-2" /> Cột: MSSV hoặc Mã sinh viên</li>
                        <li className="flex items-center"><div className="h-1 w-1 bg-blue-400 rounded-full mr-2" /> Cột: HoTen hoặc Họ và tên</li>
                        <li className="flex items-center"><div className="h-1 w-1 bg-blue-400 rounded-full mr-2" /> Các cột khác: Lop, Khoa, Email, SoDienThoai</li>
                        <li className="flex items-center font-bold mt-2 text-orange-600"><AlertCircle className="h-3 w-3 mr-1" /> Giới hạn: 10 yêu cầu / 15 phút (do bảo mật hệ thống)</li>
                      </ul>
                    </div>
                  </div>
                ) : (
                  <div className="space-y-6">
                    {isImporting && (
                      <div className="space-y-4">
                        ...
                        <p className="text-center text-xs text-gray-400 font-bold">
                          Đã xử lý {importProgress.current} / {importProgress.total} sinh viên
                        </p>
                      </div>
                    )}
                    ...
                    {importLogs.length > 0 && (
                      <div className="space-y-4">
                        <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest">Kết quả chi tiết:</h3>
                        <div className="border border-gray-100 rounded-2xl overflow-hidden max-h-60 overflow-y-auto">
                          <table className="w-full text-left text-xs">
                            <thead className="bg-gray-50 border-b border-gray-100 sticky top-0">
                              <tr>
                                <th className="px-4 py-3 font-black text-gray-400">MSSV</th>
                                <th className="px-4 py-3 font-black text-gray-400">Họ tên</th>
                                <th className="px-4 py-3 font-black text-gray-400">Kết quả</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50">
                              {importLogs.map((log, idx) => (
                                <tr key={idx} className="hover:bg-gray-50">
                                  <td className="px-4 py-3 font-bold text-gray-700">{log.studentId}</td>
                                  <td className="px-4 py-3 font-medium text-gray-600">{log.fullName}</td>
              ...
              <div className="p-10 border-t border-gray-50 flex-shrink-0">
                <Button 
                  variant="outline" 
                  onClick={() => {
                ...
                >
                  {importLogs.length > 0 ? 'Đóng' : 'Hủy bỏ'}
                </Button>
              </div>
            </motion.div>
          </div>
