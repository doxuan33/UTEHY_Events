    ...
    >
      ...
      <Link to={`/events/${event.id}`} className="block relative aspect-video overflow-hidden">
        ...
        <div className="absolute top-4 left-4">
          <div className="bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-xl shadow-sm flex flex-col items-center min-w-[50px]">
            <span className="text-[10px] font-bold text-blue-600 uppercase">{format(new Date(event.start_time), 'MMM', { locale: vi })}</span>
      ...
      <div className="p-5">
        <Link to={`/events/${event.id}`} className="block group">
          <h3 className="text-xl font-black text-gray-900 group-hover:text-blue-600 transition-colors line-clamp-2 mb-3">
            {event.title}
          </h3>
        </Link>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-50">
          <Link to={`/events/${event.id}`} className="flex-1">
            <button className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-sm transition-all shadow-lg shadow-blue-100 flex items-center justify-center">
              Xem chi tiết & Đăng ký
              <ChevronRight className="h-4 w-4 ml-1" />
            </button>
          </Link>
          <button className="ml-3 p-3 text-gray-400 hover:bg-gray-50 rounded-xl transition-colors border border-gray-100">
